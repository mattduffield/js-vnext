  ...
  test() {
    ...
    function* getPrisoners({ name, generator }) {
      ...
      for(;;) {
        ...
        const prisoner = prisoners.next([...stats]).value
    ...
    function test(...candidates) {
      ...
      // ))))
      return Object.assign({}, ...tested.map(criminal => (
        getScore(criminal.next(true)
      ))))
      // return {...tested.map(criminal => (
      //   getScore(criminal.next(true)
      // )))};
    }

    const result = test(retaliate, never, always, rand);
